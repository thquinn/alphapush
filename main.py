...
while True:
    ...
    for batch in range(batches_per_iteration):
        ...
        policy_masks = (training_policies != 0).float()
        predicted_policy = predictions[:,1:] * policy_masks
        predicted_policy = nn.functional.softmax(predicted_policy, dim=1)
        loss_policy = torch.mean(torch.sum(torch.abs(predicted_policy - training_outputs[:,1:]), dim=1))
