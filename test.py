...
def versus():
    old_net = NullNet()
    old_net = torch.load('model_270K_v000.pt', weights_only=False).cpu().eval()
    new_net = torch.load('model_270K_v008.pt', weights_only=False).cpu().eval()
...
def debug_state():
    # net = NullNet()
    net = torch.load('model_270K_v008.pt', weights_only=False).cpu().eval()
    ...
    # state = PFState.construct('......wWwW.WB.....BBb.b...', True, 2, 12) # 'squeeze': v3-6 saves @64K, 6fix@32K, v8@16K, v8pf@8K
    ...
    # state = PFState.construct('.....WwWw..WbBBbB.........', True, 2, 16) # 'squeeze2': v6 saves @32K, 6fix@16K, v8@8K, v8pf@4K
    ...
    state = PFState.construct('......wW.Ww.W.Bb.....Bb.B.', True, 2, -1) # 'duh': win-in-two, v8 needs 8K
