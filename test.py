...
def versus():
    ...
    new_net = NullNet() # torch.load('model_320K_bad.pt', weights_only=False).cpu().eval()
    eval_match(old_net, new_net, evals_per_position=10240, verbose=True)
...
def debug_state():
    net = NullNet() # torch.load('model_320K_bad.pt', weights_only=False).cpu().eval()
